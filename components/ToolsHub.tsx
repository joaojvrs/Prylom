...
const ToolsHub: React.FC<Props> = ({ onBack, t, lang, currency }) => {
  ...
  const historicalAvg = commodity === 'soja' ? 16 : 45;
    ...
    try {
      ...
      const analyze = async (locInfo: string, lat?: number, lng?: number) => {
        const response = await ai.models.generateContent({
          ...
          contents: `Atue como um Product Designer e Consultor Sênior de agronegócio Prylom. Localização alvo: ${locInfo}. Forneça um insight curto no tom Prylom. Divida em Parte 1 (Técnica) e Parte 2 (Simples) com "|||". Responda em ${lang}.`,
        });
        const [technical, simple] = (response.text || "").split("|||").map(s => s.trim());
        setLocalInsight({
          technical: technical || response.text || "",
          simple: simple || "",
          coords: lat ? `${lat.toFixed(4)}, ${lng?.toFixed(4)}` : "Coordenadas Regionais",
          locationName: locInfo
        });
      };
      ...
      if (specificLocation) {
        ...
        navigator.geolocation.getCurrentPosition(async (pos) => {
          ...
          await analyze("Brasil - Hub Central");
        });
      }
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="max-w-7xl mx-auto w-full px-4 py-8 md:py-12 animate-fadeIn flex flex-col gap-8 pb-32">
         ...
         <form onSubmit={handleManualSearch} className="flex-1 w-full flex flex-col md:flex-row items-end gap-4">
            <div className="flex-1 w-full space-y-2">
               ...
               <input 
                 ...
                 className="w-full p-4 bg-gray-50 border-2 border-transparent focus:border-prylom-gold rounded-2xl outline-none font-bold text-prylom-dark"
               />
            </div>
            <button type="submit" className="bg-prylom-dark text-white font-black px-8 py-4 rounded-2xl text-xs uppercase tracking-widest hover:scale-105 active:scale-95 transition-all w-full md:w-auto">
               Analisar
            </button>
         </form>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white rounded-[2.5rem] p-8 md:p-10 shadow-sm border border-gray-100 flex flex-col gap-8">
          <header className="flex justify-between items-start">
            <div>
              <h3 className="text-xl font-black text-prylom-dark uppercase tracking-tight mb-1">{t.greenCurrency}</h3>
              <p className="text-gray-700 text-xs font-bold">Monitor de Margem Operacional</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-prylom-dark/60 uppercase tracking-widest block px-1">Cultura</label>
              <select value={commodity} onChange={e => setCommodity(e.target.value)} className="w-full p-4 bg-gray-50 rounded-2xl font-bold text-prylom-dark outline-none border-2 border-transparent focus:border-prylom-gold">
                <option value="soja">{t.soy}</option>
                <option value="milho">{t.corn}</option>
              </select>
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-prylom-dark/60 uppercase tracking-widest block px-1">{t.bagPriceLabel}</label>
              <input type="number" value={commodityPrice} onChange={e => setCommodityPrice(parseFloat(e.target.value))} className="w-full p-4 bg-gray-50 rounded-2xl font-black text-prylom-dark outline-none" />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-prylom-dark/60 uppercase tracking-widest block px-1">Insumo ({getSymbol()} / un)</label>
              <input type="number" value={inputCost} onChange={e => setInputCost(parseFloat(e.target.value))} className="w-full p-4 bg-gray-50 rounded-2xl font-black text-prylom-dark outline-none" />
            </div>
          </div>
          ...
          <div className="bg-prylom-dark text-white rounded-[2rem] p-8 flex flex-col md:flex-row items-center gap-8 shadow-2xl relative overflow-hidden">
             <div className="flex-1 text-center md:text-left z-10">
                ...
                <p className="text-3xl md:text-4xl font-black leading-tight">
                  {barterRatio.toFixed(1)} 
                  <span className="text-sm text-gray-300 font-bold uppercase ml-2 block md:inline">{t.bagsPerUn}</span>
                </p>
                <p className="text-[10px] text-gray-400 mt-2 font-medium">{t.histAvg}: {historicalAvg} sacas</p>
             </div>
             ...
             <div className="flex-1 text-center md:text-left z-10">
                ...
                <p className="text-[10px] text-gray-400 mt-2 font-medium">{t.minProduction}</p>
             </div>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-[2.5rem] p-8 border border-gray-100 flex flex-col gap-6 shadow-inner">
          <div>
            ...
            <p className="text-gray-700 text-xs font-bold">Fonte: IMEA / CONAB (Estimado)</p>
          </div>
          
          <div className="space-y-4">
             <label className="text-[10px] font-black text-prylom-dark/60 uppercase tracking-widest block px-1">{t.selectRegion}</label>
             <select value={region} onChange={e => setRegion(e.target.value)} className="w-full p-4 bg-white rounded-2xl font-bold text-prylom-dark border border-gray-200 outline-none">
          ...
          <div className="mt-4 space-y-4 flex-1">
             <div className="flex justify-between items-center p-4 bg-white rounded-2xl shadow-sm">
               <span className="text-xs font-bold text-gray-700">{t.avgCostHa}</span>
               <span className="font-black text-prylom-dark">{formatPrice(regionalCosts[region]?.costHa, 0)}</span>
             </div>
             <div className="flex justify-between items-center p-4 bg-white rounded-2xl shadow-sm">
               <span className="text-xs font-bold text-gray-700">{t.regionalAptitude}</span>
               <span className="text-[10px] font-black uppercase text-prylom-gold">{t.highPerformance}</span>
             </div>
          </div>
        </div>
      </div>
